...
module.exports = grammar({
  ...
  rules: {
    ...
    source_file: ($) => seq(optional($.description_block), repeat($.module_block)),
    ...
    module_block: ($) =>
      prec.right(seq("@module", field("name", $.identifier), repeat($._module_item))),

    // Items in a module - can be description, features, requirements, or annotations
    _module_item: ($) =>
      choice($._block_content, $.feature_block, $.requirement_block, $.depends_on, $.constraint),

    // @feature - user-facing capabilities within a module
    feature_block: ($) =>
      prec.right(seq("@feature", field("name", $.identifier), repeat($._feature_item))),

    // Items in a feature - can be description, requirements, or annotations
    _feature_item: ($) => choice($._block_content, $.requirement_block, $.depends_on, $.constraint),

    // @requirement - specific, implementable units of functionality
    requirement_block: ($) =>
      prec.right(seq("@requirement", field("name", $.identifier), repeat($._requirement_item))),

    // Items in a requirement - can be description or annotations
    _requirement_item: ($) => choice($._block_content, $.depends_on, $.constraint),
    ...
    depends_on: ($) => seq("@depends-on", $.reference, repeat(seq(",", $.reference))),

    // @constraint - implementation requirements that must be satisfied
    constraint: ($) =>
      prec.right(seq("@constraint", field("name", $.identifier), repeat($._block_content))),
    ...
    reference: ($) => prec.left(seq($.identifier, repeat(seq(".", $.identifier)))),
    ...
    // Fenced code block: ``` ... ```
    code_block: ($) =>
      seq("```", optional(field("language", $.code_language)), optional($.code_content), "```"),
