...
- [x] **Import `ReferenceNode` type in ast.test.ts** - The test file tests `ReferenceNode` properties but doesn't import the type. Add to imports for type safety. (Completed: Added `type ReferenceNode` to imports from `../src/ast` and added explicit type annotations to `ref1` and `ref2` variables in the "transforms depends-on with deeply nested reference" test to ensure type safety.)
